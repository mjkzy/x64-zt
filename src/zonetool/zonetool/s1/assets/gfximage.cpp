...
namespace zonetool::s1
{
	...
	{
		const auto asset_path = utils::string::va("streamed_images\\%s.s1Image", clean_name(name).data());
	...
	{
		auto path = "images\\" + clean_name(name) + ".s1Image";
	...
	void dump_image_dds(GfxImage* image)
	{
		if (image->streamed)
		{
			dump_streamed_image(image, stream_files[*stream_file_index].fileIndex == 96, true);
			return;
		}
