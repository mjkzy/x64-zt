...
#define DUMP_ASSET_CONVERT(__type__,__namespace__,__struct__) \
		...
		try
		{
			...
			DUMP_ASSET_CONVERT(ASSET_TYPE_RAWFILE, rawfile, RawFile);
...
#define DUMP_ASSET_CONVERT_CONVERT(__type__, __namespace__, __struct__) \
		...
		try
		{
			...
			DUMP_ASSET_CONVERT_NO_CONVERT(ASSET_TYPE_RAWFILE, rawfile, RawFile);
...
#define DUMP_ASSET(__type__,___,__struct__) \
	...
	void register_commands()
	{
		...
		::iw6::command::add("dumpmatchingzones", [](const ::iw6::command::params& params)
		{
			if (params.size() < 2)
			{
				ZONETOOL_ERROR("usage: dumpzone <zone>");
